...
if len(sys.argv) < 2:
    sys.exit("Use: python train_val_test_SHINEreg.py result.csv -c config/config.tcga.SHINEreg.yaml")
...
if os.path.isfile(fn_G):
    print(f'Loading {fn_G}')
    f = open(fn_G, 'rb')
    [G] = pkl.load(f)
    f.close()
else:
    print(f'Computing G...')
    H = hgo.construct_H(fn_H)
...
if scale:
    scaler = StandardScaler().fit(m.iloc[train_idx])
    m.iloc[train_idx] = scaler.transform(m.iloc[train_idx])
    m.iloc[val_idx] = scaler.transform(m.iloc[val_idx])
    m.iloc[test_idx] = scaler.transform(m.iloc[test_idx])
                    ...
                    for weight_decay in weight_decay_list:
                        for seed in seeds:
                            ...
                            if os.path.isfile(f'{fn}.ckpt'):
                                ...
                                model = model.fit(nfts, m, cf, y, cls_loss, nratio, optimizer, schedular, cfg['max_epoch'], print_freq=cfg['print_freq']) 
